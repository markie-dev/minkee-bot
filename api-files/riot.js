...
async function getMatchData(gameId, summonerId, champId) {
  ...
  let participant = body.participants.filter(
    (participant) => participant.championId === champId
  )[0];
  ...
  let cs = (
    ...
    (body.gameDuration / 60.0)
  ).toFixed(2);

  let currentRank = await axios.get(
  ...
  let gameData = {
    ...
    win: isVictor,
    currentRank:
      tier === "DIAMOND" ? `${tier} ${rank} ${lp} LP` : `${tier} ${lp} LP`,
    WL: winloss,
